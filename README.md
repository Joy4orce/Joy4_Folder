...
5. **중복 폴더 정리** — 하위 폴더들을 탐색해, **내용이 겹치는** 형제 폴더(같은 트랙의 다른 버전)를 찾아 불필요한 쪽을 **휴지통으로** 이동합니다. 판단 규칙:
   ...
   - "내용이 달라도 이름이 짝이면 확인" 옵션(`dedup_name_only`)을 켜면, 파일 이름이 정확히 같지 않아 자동 판단(겹침 50% 이상)에는 못 미쳐도 이름이 짝(`있음`/`없음`, `mp3`/`wav` 등 양쪽 마커가 모두 있는 경우)이면 자동 삭제 대신 **팝업으로 띄워** 직접 선택하게 합니다. 홍수처럼 뜨지 않도록 양쪽에 마커가 다 있을 때만 후보로 잡습니다.
   ...
   - **겹치는 파일이 없으면 삭제하지 않음**: 삭제 규칙에 해당하는 폴더라도, 유지될 폴더와 겹치는 파일이 하나도 없으면(그 폴더의 파일이 어디에도 중복되지 않으면) 자동 삭제도, 팝업 후보로도 올리지 않습니다. 고유한 파일이 사라지는 것을 막기 위한 장치입니다.
   - **안전장치**: 자동 삭제는 유지될 폴더에 파일이 실제로 겹치는 경우에만 적용하고, 삭제 전 확인 창을 거치며, 영구 삭제가 아니라 **휴지통**으로 이동하므로 복구할 수 있습니다.
